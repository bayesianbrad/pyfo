...
from pyfo.distributions.util import get_probs_and_logits, torch_eye, torch_multinomial, torch_zeros_like
...
class Categorical(Distribution):
    ...
    def __init__(self, ps=None, vs= None, logits=None, batch_size=None, log_pdf_mask=None, *args, **kwargs):
        ...
        self.ps, self.logits = get_probs_and_logits(ps=VariableCast(ps), logits=VariableCast(logits),
                                                    is_multidimensional=True)
        # vs is None, Variable(Tensor), or numpy.array
        self.log_pdf_mask = log_pdf_mask
        if vs is not None:
            self.vs = VariableCast(vs)
            if self.vs.size() != self.ps.size():
                raise ValueError("Expected vs.size() or vs.shape == ps.size(), but got {} vs {}".format(self.vs.size(), self.ps.size()))
        else:
            self.vs= None
