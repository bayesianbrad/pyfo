...
(defn wrap-fn [args body]
  ;;(println "wrapping function of" args ":\n" body "\n")
  (let [args (into [] args)]
...
(defn partial-eval
  ...
  Updated 21.8.2017 to make this more conservative: it will only operate
  on data structures containing boxed expressions or variables, not directly
  on boxed expressions."
  [expr]
  (let [is-fn-call (and (seq? expr) (not (expression? (second expr))))
  ...
  (cond (vector? expr)
        ...
        (case (name (first expr))
          ...
          "observe" (let [[e1 e2] (rest expr)
                          ...
                          y (Variable. (gensym "y"))
                          ;; F1 [(into [] (freevars E1)) E1] ;; What??
                          ;; TODO: do we actually partially evaluate here?
                          ;; TODO: should this be Score(E1)?
                          ;; TODO: is passing back 'nil acceptable if phi is false?
                          F (if (= phi true) E1 `(if ~phi ~E1))
                          F [(into [] (freevars F)) F]
                          Z (clojure.set/difference (freevars F) #{y})
                          B (into #{} (for [z Z] [z y]))]
                      [[(union V #{y}) (union A B) (assoc P y F) (assoc O y E2)] E2])
...
(defn freevars-match-arcs?
  ...
  (let [[V A P O] G
    ...
    (let [nodes (into {} (for [[a b] nodes] [a @b]))]
      ;;(println "matches?"
      ;;         (for [[v pv] P] [v (= (freevars pv) (get nodes v #{}))]))
      (reduce #(and %1 %2) true
              (for [[v pv] P]
...
(defn print-graph
  ...
  (let [[V A P O] G]
    ...
    (println "\nObserved values O:")
    (println (clojure.string/join "\n" (for [[k v] O] (str k " -> " v)))))) ;;(:value v) "; " (:cond v)))))))
    ...
    (loop [logprob 0.0
      ...
      (if (empty? nodes)
        ...
        (let [address (first nodes)
          ...
          ;; Check if if this is an observe
          (if (contains? O address)
            (recur (+ logprob
                      ;; if 'dist is 'nil, then do not include in the trace
                      (if dist
                        (anglican.runtime/observe* dist (O address))
                        0.0))
                   (rest nodes))
